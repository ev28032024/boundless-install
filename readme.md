...
### 7. Запуск Bento
>Bento — это часть инфраструктуры, отвечающий за создание доказательств выполнения вычислений без раскрытия входных данных. Она запускается на GPU-совместимых серверах и используется для генерации криптографических доказательств (proofs).

### Запуск Bento
>Эта команда запускает Bento-процесс в фоновом режиме, без подключения к Broker. Он будет ждать входящие задачи для генерации доказательств.
```
just bento
```

### Проверка логов
>Позволяет следить за тем, как Bento запускается, и удостовериться, что нет ошибок.
'''
just bento logs
'''

### Запуск тестового пруфа
```
RUST_LOG=info bento_cli -c 32
```
>Опция -c 32 означает "32 задачи на вычисление".
>Если всё прошло успешно, будет сообщение вроде Job Done!.
